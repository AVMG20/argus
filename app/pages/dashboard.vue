  ...
  <UDashboardPanel id="projects">
    <template #header>
      <UDashboardNavbar title="Projects">
        ...
        <template #right><UButton to="/projects/new" icon="i-lucide-plus" label="New project" /></template>
      </UDashboardNavbar>
    </template>
    <template #body>
      <div v-if="loading" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><USkeleton v-for="index in 3" :key="index" class="h-40" /></div>
      <UEmpty
      ...
      <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <NuxtLink v-for="item in projects" :key="item.id" :to="`/projects/${item.id}`" class="group rounded-lg border border-default bg-elevated p-5 transition hover:border-primary/50 hover:shadow-sm">
          <div class="flex items-start justify-between">
            <span class="grid size-10 place-items-center rounded-lg bg-primary/10 text-primary"><UIcon name="i-lucide-box" class="size-5" /></span>
            <UBadge v-if="item.unresolvedCount" color="error" variant="subtle" size="sm">{{ item.unresolvedCount }} unresolved</UBadge>
            <UBadge v-else color="success" variant="subtle" size="sm">Healthy</UBadge>
          </div>
